...
estimatedMinutes: 7
prerequisites: []
summary: "One queen per row reduces the problem from n² cells to n column choices per row. Three guard sets — columns, ↘ diagonals (r-c), ↙ diagonals (r+c) — give O(1) conflict checks."
complexity:
  time: "O(n!)"
  space: "O(n)"
commonPitfalls:
...
- time **O(n!)** with diagonal pruning
- space **O(n)** path + 3 guard sets
...
## Practice — LeetCode
- [51. N-Queens (Hard)](https://leetcode.com/problems/n-queens/) — return all boards.
- [52. N-Queens II (Hard)](https://leetcode.com/problems/n-queens-ii/) — count only, same backtracking.
- [37. Sudoku Solver (Hard)](https://leetcode.com/problems/sudoku-solver/) — same idea, three constraint sets.
- [36. Valid Sudoku (Medium)](https://leetcode.com/problems/valid-sudoku/) — checker that powers the solver.
- [980. Unique Paths III (Hard)](https://leetcode.com/problems/unique-paths-iii/) — backtrack with cell mask.
- [1239. Maximum Length of a Concatenated String with Unique Characters (Medium)](https://leetcode.com/problems/maximum-length-of-a-concatenated-string-with-unique-characters/) — bitmask backtracking.
- [698. Partition to K Equal Sum Subsets (Medium)](https://leetcode.com/problems/partition-to-k-equal-sum-subsets/) — assign items to k bins.
- [473. Matchsticks to Square (Medium)](https://leetcode.com/problems/matchsticks-to-square/) — k=4 partition variant.
- [1255. Maximum Score Words Formed by Letters (Hard)](https://leetcode.com/problems/maximum-score-words-formed-by-letters/) — include/exclude with letter budget.
- [1849. Splitting a String Into Descending Consecutive Values (Medium)](https://leetcode.com/problems/splitting-a-string-into-descending-consecutive-values/) — string-partition backtracking.
