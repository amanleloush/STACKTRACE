...
estimatedMinutes: 6
prerequisites: []
summary: "Big-O describes how runtime or space scales with input size n, ignoring constants. It is an upper bound that interviewers expect you to state for both time and space before coding."
commonPitfalls:
  - "`s.contains(x)` on a Python list is O(n); use a set for O(1)."
