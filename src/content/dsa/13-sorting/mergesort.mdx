...
estimatedMinutes: 6
prerequisites: []
summary: "Divide and conquer — recursively halve, then merge two sorted halves in linear time. The canonical O(n log n) stable sort; basis of Python's Timsort and Java's Arrays.sort for objects."
complexity:
  time: "O(n log n)"
  space: "O(n)"
commonPitfalls:
  - "Allocating new lists at every level doubles peak memory — production implementations preallocate one O(n) buffer and ping-pong."
  - "Using `<` instead of `<=` in the merge breaks stability when keys tie — always take from the left first on equality."
  - "Forgetting the `out.extend(L[i:])` tail copy — one side will have leftovers when the other empties."
  - "Base case must handle `len(a) <= 1`, not just `len(a) == 0`."
relatedAnims: []
---
...
- [Sort an Array (Med)](https://leetcode.com/problems/sort-an-array/) — implement from scratch.
- [Sort List (Med)](https://leetcode.com/problems/sort-list/) — merge sort on a linked list.
- [Merge Sorted Array (Easy)](https://leetcode.com/problems/merge-sorted-array/) — the merge step in isolation.
- [Merge k Sorted Lists (Hard)](https://leetcode.com/problems/merge-k-sorted-lists/) — k-way merge with a heap or pairwise.
- [Merge Two Sorted Lists (Easy)](https://leetcode.com/problems/merge-two-sorted-lists/) — linked-list merge.
- [Count of Smaller Numbers After Self (Hard)](https://leetcode.com/problems/count-of-smaller-numbers-after-self/) — count inversions during merge.
- [Count of Range Sum (Hard)](https://leetcode.com/problems/count-of-range-sum/) — merge sort on prefix sums.
- [Reverse Pairs (Hard)](https://leetcode.com/problems/reverse-pairs/) — variant of counting inversions.
- [Median of Two Sorted Arrays (Hard)](https://leetcode.com/problems/median-of-two-sorted-arrays/) — virtual merge with binary search.
- [Intersection of Three Sorted Arrays (Easy)](https://leetcode.com/problems/intersection-of-three-sorted-arrays/) — three-pointer merge.
