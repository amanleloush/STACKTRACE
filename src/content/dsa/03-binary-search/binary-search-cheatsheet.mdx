...
estimatedMinutes: 5
prerequisites: []
summary: "One-page recall for binary search: triggers, three skeletons, six variants, and the bugs that always bite."
commonPitfalls:
  - "Infinite loop: `lo = mid` (not `mid + 1`) with two elements never shrinks. Pick a flavor and stick."
  - "Integer overflow in Java/C++: use `lo + (hi - lo) // 2`. Habit-build in Python too."
  - "Wrong return: classic returns `-1`; `lower_bound` returns insertion point (could be `n`)."
  - "Off-by-one on `hi`: exclusive (`len`) for `lower_bound`, inclusive (`len - 1`) for classic."
  - "Rotated array with dupes (LC 81) degrades to O(n) worst case."
  - "Parametric search direction — sketch the predicate first."
relatedAnims: []
---
...
### Binary search on answer — integer √n

<Anim id="bsearch-on-answer" />

### Search 2D Matrix — staircase from top-right
