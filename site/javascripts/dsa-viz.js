...
(function () {
  ...
  // ---------- mount on DOM ready ---------------------------------------
  function mountAll() {
    document.querySelectorAll(".dsa-viz[data-algo]").forEach(host => {
      if (host.dataset.mounted) return;
      try {
        mount(host);
        host.dataset.mounted = "1";
      } catch (e) {
        console.error("dsa-viz error", host.dataset.algo, e);
        host.innerHTML = `<p style="color:#ef4444;font-family:monospace">Visualization failed: ${e.message}</p>`;
      }
    });
  }
  ...
  if (window.document$) {
    window.document$.subscribe(() => {
      document.querySelectorAll(".dsa-viz[data-algo]").forEach(h => delete h.dataset.mounted);
      boot();
    });
  }
})();
